...
public class PlayerSystem : EntityProcessingSystem
{
    ...
    public PlayerSystem()
        : base(aspectBuilder: Aspect.All(typeof(Body), typeof(Player), typeof(Transform2), typeof(AnimatedSprite)))
    { }
    ...
    public override void Process(GameTime gameTime, int entityId)
    {
        ...
        AnimatedSprite        sprite        = _spriteMapper.Get(entityId);
        ...
        if (!player.IsAttacking)
        {
            ...
            if (body.Velocity.EqualsWithTolerence(Vector2.Zero, tolerance: 5))
            {
        ...
        switch (player.State)
        {
            ...
            case State.Kicking:
                sprite.Play(name: "kick", () => player.State = State.Idle);

                break;

            case State.Punching:
                sprite.Play(name: "punch", () => player.State = State.Idle);

                break;

            case State.Cool:
                sprite.Play(name: "cool");
